...
trait MatchSort
{
    ...
    public function selectMatch(){
        ...
        $latest_match = Match::whereRaw("TIMESTAMPDIFF(MINUTE, match_day, NOW()) > 120")->orderBy('match_day','desc')->limit((int)$count_latest_match_scoreboard->value)->get();
        $live_match = Match::whereRaw("(TIMESTAMPDIFF(MINUTE, match_day, NOW()) < 120) AND (TIMESTAMPDIFF(MINUTE, NOW(), match_day) < 5)")->orderBy('match_day','desc')->limit((int)$count_live_match_scoreboard->value)->get();
        $upcoming_matches = Match::whereRaw("TIMESTAMPDIFF(MINUTE, NOW(), match_day) > 5")->orderBy('match_day','desc')->limit((int)$count_upcoming_match_scoreboard->value)->get();
